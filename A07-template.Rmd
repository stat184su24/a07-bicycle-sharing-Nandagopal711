...
Trips_filtered %>%
  ...
  geom_boxplot() +
  labs(title = "Distribution of Bike Rental Durations by the type of Client",
       x = "Client Type",
...
```{r}
Trips_dty <- Trips %>%
  mutate(day_of_year = as.numeric(format(sdate, "%j")))
Trips_dty %>%
  ggplot(aes(x = day_of_year)) +
  geom_histogram(binwidth = 1, fill = "blue", color = "black") +
  labs(title = "Bike Usage by Day of the Year",
       x = "Day of the Year",
       y = "Count") +
  theme_minimal()
```
...
It can be noted that this service starts from October. It has its most users on its 275th day. It has its least users on its 335th day
...
```{r}
Trips_dow <- Trips %>%
  mutate(day_of_week = factor(format(sdate, "%A"), levels = c("Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday")))
Trips_dow %>%
...
I decided to go with a bar graph as opposed to a histogram or a density plot here cause it is the best form of visualization for these categories. It has the highest use in a week on Thursdays and the least use on Sundays.
...
```{r}
Trips_hour <- Trips %>%
  mutate(hour_of_day = as.numeric(format(sdate, "%H")))

Trips_hour %>%
  ggplot(aes(x = hour_of_day)) +
  geom_histogram(binwidth = 1, fill = "green", color = "black") +
  labs(title = "Histogram of Bike Usage by Hour of the Day",
       x = "Hour of the Day",
       y = "Count") +
  theme_minimal()
```
...
We can see at hour 17 of the day is when the bikes are most used.
They are least used at hour 4 of the day.

After the first 5-6 hours of the day, they seem to be used extensively.

### Minute of Hour

```{r}
Trips_minute <- Trips %>%
  mutate(minute_of_hour = as.numeric(format(sdate, "%M")))

Trips_minute %>%
  ggplot(aes(x = minute_of_hour)) +
  geom_histogram(binwidth = 1, fill = "red", color = "black") +
  labs(title = "Histogram of Bike Usage by Minute of the Hour",
...
They are used the most on the 43rd minute of the hour.
They are used the least in what seems to be the 60th minute of the hour. 
However, they seem to be used uniformly throughout the hour.
...
Trips_grouped <- Trips %>%
  mutate(hour_of_day = as.numeric(format(sdate, "%H")),
         day_of_week = factor(format(sdate, "%A"),
                              levels = c("Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday")))

Trips_summary <- Trips_grouped %>%
  group_by(hour_of_day, day_of_week, client) %>%
  summarise(total_events = n(), .groups = 'drop')

ggplot(Trips_summary, aes(x = hour_of_day, y = total_events, color = client)) +
  geom_line() +
  facet_wrap(~ day_of_week) +
  labs(title = "Total Bike Rentals by Hour of the Day",
       x = "Hour of the Day",
...
We can see from Mondays through Fridays, the bikes are most used between hours 5-10 and hours 15-20. On Sundays and Saturdays they are most used hours 10 - 17.5.

There are a lot more registered clients using the bikes everyday than there are casual clients.
...
Trips_with_distances %>%
  ...
  theme_minimal()
```
This plot shows that the inner joined data frame's distances with the trips dataset are easier to plot and most of their trips are about 1 km in distance and they mostly stay within 1-5 kms.

```{r}
PairDistances %>%
  ggplot(aes(x = distance)) +
  geom_histogram(binwidth = 0.5, fill = "red", color = "black") +
  labs(title = "Distribution of Pairwise Station Distances",
       x = "Distance (km)",
       y = "Count") +
  theme_minimal()
```
Paired dataset shows that while most of the trips are between 1-10 kms, a lot of them can go upto the 30s(km). 

There is a massive difference in scale between the two.
...
The starting and ending stations are identified by various names in Simple and Simple2. This keeps things clear when merging the data. Additionally, it guarantees that the right coordinates are utilized while calculating distance. It is obvious which latitude and longitude correspond to the initial station and which to the final station due to the various designations.
